#!/usr/bin/env bash
# Builds an isolated "Cadence Dev" debug APK for side-by-side testing
#
# (c) Copyright 2026 Liminal HQ, Scott Morris
# SPDX-License-Identifier: Apache-2.0 OR MIT

# Builds an isolated "Cadence Dev" debug APK (ca.liminalhq.cadence.dev), installable
...
# runs 800MB+ unstripped. Override with:
#   CADENCE_DEV_TARGET=universal scripts/build-android-dev.sh
#
...
# checks can still reach for an older Build-Tools/Platform baseline that isn't in the image.
# Without a persistent volume here, `--rm` throws that away every run, so it re-downloads
# from Google's servers on every single build instead of once.

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
IMAGE="ghcr.io/liminal-hq/tauri-dev-mobile:latest"
DEV_IDENTIFIER="ca.liminalhq.cadence.dev"
DEV_CONFIG="{\"identifier\":\"$DEV_IDENTIFIER\",\"productName\":\"Cadence Dev\"}"
...
docker run --rm \
	...
	-v cadence-android-gradle-cache:/home/vscode/.gradle \
	-v cadence-android-cargo-cache:/home/vscode/.cargo/registry \
	-v cadence-android-keystore:/home/vscode/.android \
	-v cadence-android-sdk-extras:/home/vscode/Android/Sdk \
	-w /workspace \
	"$IMAGE" \
	bash -c "
		set -e
...
APK_DEST="$OUT_DIR/cadence-dev-$(date +%Y%m%d-%H%M).apk"
cp "$APK_SRC" "$APK_DEST"

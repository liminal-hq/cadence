...
/// A routine-authored superset template; materialization copies these into workout-level
/// supersets, matching how `SetTemplate` materializes into `Set` (schema comment, 0001 migration).
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[cfg_attr(test, derive(ts_rs::TS))]
#[cfg_attr(test, ts(export, export_to = "../../src/domain/generated/"))]
#[serde(rename_all = "camelCase")]
...
/// Exactly one of `values` or `population_rule` is meaningful for a given template; both are
/// optional at the type level because the DB column set is shared, not because either is expected
/// alongside the other.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[cfg_attr(test, derive(ts_rs::TS))]
#[cfg_attr(test, ts(export, export_to = "../../src/domain/generated/"))]
...
pub struct SetTemplate {
    ...
    /// The only value shipped in v1 is `"seed-last-performance"`; `None` means the explicit
    /// target values above are used as-is.
    #[cfg_attr(test, ts(optional))]
...
/// The only `population_rule` value set_templates support in v1 — validated in the repo layer
/// rather than a DB `CHECK` constraint, matching `exercises.category`'s "open TEXT, Rust-validated"
/// convention, since more rules are expected later.

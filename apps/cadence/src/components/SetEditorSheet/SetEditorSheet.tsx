	...
	return (
		<BottomSheet onClose={onClose} ariaLabel={`Edit set ${set.order}`}>
			{showDraftBanner && (
				<Banner
					icon="restore"
					message="Draft restored · not logged"
					tone="neutral"
					action={{ label: 'Discard', onClick: () => setShowDraftBanner(false) }}
				/>
			)}
			...
			<div className="set-editor-sheet__header">
				...
				<IconButton icon="close" label="Close" size="large" onClick={onClose} />
			</div>
			...
			<SegmentedControl
				...
				value={draft.status}
				onChange={(status) => (status === 'planned' ? setPlanned() : setCompleted())}
			/>
			...
			<div className="set-editor-sheet__footer">
				...
				<div className="set-editor-sheet__footer-actions">
					...
					<Button variant="filled" onClick={commitAndClose} disabled={durationInvalid}>

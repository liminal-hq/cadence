...
Each routine exercise may also carry an optional rest override, scoped to the routine template only — it is not carried into materialization or a live workout's rest timer, matching how superset auto-advance/rest behaviour is already scoped away from routine templates elsewhere in this section. Adding a set to an exercise can duplicate the most recently added template's own values rather than re-entering them. If an exercise a routine references has since been deleted from the library, the editor surfaces it as a clearly marked, resolvable error offering removal from the section, rather than failing to load the whole routine. Editing a routine holds every change in an explicit draft, shown as unsaved, until the user saves or discards it.

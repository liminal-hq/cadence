...
Cadence has moved past the specification phase: `apps/cadence` is scaffolded (Tauri v2 + React/TypeScript, Bun-managed) with a desktop title bar, the M3 design tokens from the Claude Design handoff, and a shared app shell (top app bar + bottom navigation) wired up across all four primary destinations. `@tanstack/react-router` now backs navigation (a `/today|/history|/plan|/progress` tab layout plus sibling `/workout/$workoutId`, `/workout-exercise/$workoutExerciseId`, `/settings/*`, `/history/workout/$workoutId`, and `/exercise/$exerciseId` routes), and the P-14 Exercise logging flow is built out in full against a real Rust/Tauri/SQLite backend (an async, in-memory `LoggingRepository` mock remains for JS unit tests only, now that the live app is backed by real migrations and persistence): the "Preferred direction" fixed stepper-cluster pattern, the Set editor/Set note/expanded rest timer sheets, the plate calculator, and Priya's 5-step coach mark tour. Today's "Start workout" creates a genuine workout (P-10/P-11), Workout detail (P-12) lists its exercises, and Add exercise (P-13) is a real picker over an 80-exercise starter library — the three demo scenarios that used to be Logging's only entry point are retired along with the seed data they depended on. Threshold's `RouteStage`/`ScreenStack` "peek" screen-transition architecture is ported too, though it's inert on desktop by construction until there's an Android target to attach the native predictive-back plugin to. A shared M3 component library lives under `apps/cadence/src/components/ui/`, with every Logging component refactored onto it. The Settings hub (P-60) is built, with Units/Timers/Data/Watch/Plates/Accessibility real and the remaining rows `ComingSoon` stubs. History (P-40–P-46) is built in full: a `Workout` domain entity backs Calendar/List, workout detail with a real "Copy to today", and exercise detail's History/Graph/Records/Stats/Goals tabs (Goals is real CRUD, P-48, with progress computed live over logged history, never stored, a manual achieved toggle, an overdue state, and explicit handling for a goal whose target no longer matches the exercise's current metric profile). The Plan tab (P-30–P-32) is real: new `routines` and `categories` domain modules back a routine list, detail, and editor screen — sections, exercise ordering, and a set-template editor — and the P-20 review screen lets a user include/exclude and reorder a section's exercises, inspect each set template's resolved target (including a live "seeded from last performance" preview), and commit via the same Coordinator method that turns a routine section into a genuine workout. An exercise library/editor (P-33/P-34, real `exercises` CRUD) and a category editor (P-35, real create/rename/recolour/reorder/archive/delete, replacing the Settings stub) round out the Plan-adjacent surface; a few older screens (`DetailAppBar`, `HistoryHubScreen`, `AddExerciseSheet`) still read the hardcoded category-colour map rather than live data, tracked as follow-up. Progress is real too: new `goals`, `measurements`, and `analysis` domain modules back the Goals tab, a measurement tracker and detail/editor (P-49/P-50, with quick-log, a "Manage measurements" mode, a goal line on the graph, and a unit-change guard once a definition has records or a goal), and training analysis (P-47, Progress's landing screen — a period/metric/group-by breakdown table over every completed set with drill-down to the originating workout, plus pin/unpin over a real `analysis_favourites` CRUD surface for saving a recurring period/metric/group-by as a named, reapplicable chip). `apps/cadence-wear`, `packages/core`, and `plugins/` remain planned; the [Repository Layout](#repository-layout) and [Tauri v2](#tauri-v2) sections below still describe those as the _planned_ shape, carried over from Threshold's engineering patterns per `SPEC.md`. Update this file further (removing the remaining "planned" qualifiers) as each part is actually scaffolded.

...
class PredictiveBackPlugin(private val activity: Activity) : Plugin(activity) {
    ...
    // Registered once at startup. A back gesture can only occur while the Activity (and this
    // channel) is already live, so there's no cold-start delivery gap to bridge.
    @Command
